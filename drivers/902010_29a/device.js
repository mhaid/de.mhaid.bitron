...
class Bitron_902010_29a extends ZigBeeDevice 
{
	...
	async onNodeInit({ zclNode }) {
		...
		// Flow Actions
		let sirenActionTest = this.homey.flow.getActionCard('29A_siren_test');
		sirenActionTest.registerRunListener(async (args, state) => {
			try {
				...
				await zclNode.endpoints[1].clusters.iasWD.startWarning({warningMode: 'on,on,medium', warningDuration: 5, strobeDutyCycle: 50, storbeLevel: 'medium'});
			} catch (error) {
				console.log(error);
			}
			return true;
		...
		let sirenActionTimer = this.homey.flow.getActionCard('29A_siren_timer');
		...
		let sirenActionOn = this.homey.flow.getActionCard('29A_siren_on');
		...
		let sirenActionOff = this.homey.flow.getActionCard('29A_siren_off');
...
module.exports = Bitron_902010_29a;